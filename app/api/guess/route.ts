...
export async function POST(request: NextRequest) {
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一个图像识别专家。用户会提供一张手绘图像的base64编码，你需要分析图像内容并猜测用户画的是什么。请用简洁的中文回答，只给出猜测结果，不要解释。如果无法确定，可以给出最可能的猜测。",
          },
          {
            role: "user",
            content: [
    ...
    let guess = data.choices?.[0]?.message?.reasoning_content?.trim();
    guess += data.choices?.[0]?.message?.content?.trim() || "无法识别";
...
function getMockGuess(): string {
  ...
  return mockGuesses[Math.floor(Math.random() * mockGuesses.length)] + "(mock)";
}
...
export async function GET() {
  return NextResponse.json({
    ...
    instructions:
      '发送POST请求，包含{ image: "base64图像数据", model: "模型名称(可选)" }',
    config: {
